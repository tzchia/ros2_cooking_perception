"""YOLO dataset export helpers with Multi-Class Support."""
...
def mask_to_polygons_multiclass(mask_img: np.ndarray) -> Dict[int, List[np.ndarray]]:
    try:
        import cv2
    except ImportError as exc:
        raise ImportError("OpenCV required.") from exc
    ...
    for uid in unique_ids:
        ...
        binary_mask = (mask_img == uid).astype(np.uint8) * 255
        ...
        if polys:
            results[int(uid)] = polys
            
    return results


def write_label(
    class_polys: Dict[int, List[np.ndarray]], 
    w: int, h: int, 
    label_path: Path
) -> None:
    lines = []
    for mask_id, polys in class_polys.items():
        yolo_class_id = mask_id - 1 
...
def _prepare_image_dir(dataset_dir: Path, shared_images_dir: Path, split_name: str) -> tuple[Path, bool]:
    ...
    if split_dir.is_symlink():
        if split_dir.exists(): return shared_split_dir, True
        split_dir.unlink()

    if split_dir.exists():
        if split_dir.is_dir() and not any(split_dir.iterdir()): split_dir.rmdir()
        else: return split_dir, False
...
def export_yolo(
    ...
    for split_name, split_df in split_sets:
        ...
        for _, row in split_df.iterrows():
            ...
            class_polys = mask_to_polygons_multiclass(mask_img)
            ...
            write_label(class_polys, rgb_img.shape[1], rgb_img.shape[0], label_path)
